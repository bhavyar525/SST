...
namespace ScrumStandUpTrackerProject.Repositories
{
    public class DailyStatusRepository : IDailyStatusRepository
    {
        ...
        //public async Task UpdateAsync(DailyStatus status)
        //{
        //    _logger.LogInformation("Updating DailyStatus with ID: {Id}", status.Id);
        //    var existing = await _context.DailyStatuses.FindAsync(status.Id);
        //    if (existing == null)
        //    {
        //        _logger.LogWarning("DailyStatus with ID {Id} not found", status.Id);
        //        throw new KeyNotFoundException("DailyStatus not found");
        //    }
        //    // Update properties (or just mark entity as modified)
        //    _context.Entry(existing).CurrentValues.SetValues(status);
        //    await _context.SaveChangesAsync();
        //    _logger.LogInformation("DailyStatus with ID {Id} updated successfully", status.Id);
        //}

        //public async Task<DailyStatus> Update(int id, DailyStatusDTO dto)
        //{
        //    var e = await _context.DailyStatuses.FindAsync(id);
        //    if (e == null)
        //    {
        //        throw new KeyNotFoundException("DailyStatus with id is not found");
        //    }
        //    e.Id = dto.Id;
        //    e.DeveloperName = dto.DeveloperName;
        //    e.TaskDetails = dto.TaskDetails;
        //    e.DidYesterday = dto.DidYesterday;
        //    e.DoingToday = dto.DoingToday;
        //    e.Blockers = dto.Blockers;

        //    if (dto.SubmissionDate.HasValue)
        //    {
        //        e.SubmissionDate = dto.SubmissionDate.Value;
        //    }
        //    await _context.SaveChangesAsync();
        //    return e;
